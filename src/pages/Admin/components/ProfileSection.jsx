...
export default function ProfileSection() {
  ...
  return (
    <section className="profile-section" id="profile-section" aria-label="Profile">
      <h2>User Details</h2>
      <button
        type="button"
        id="peditBtn"
...
/* Backwards-compatible initializer for non-React usage. */
export function initProfileSection(root) {
  try {
    if (!root) return;
    const container = root.querySelector ? root.querySelector("#profile-section") : null;
    if (!container) return;

    // If the container appears to be under React control, do nothing.
    // (We check for presence of data-reactroot or data-reactroot attribute on nearest root - best-effort)
    if (container.dataset && container.dataset.reactroot) return;
